function [status,comment] = updrs_to_xls(Headers,DbsData,CrwData,File)
%{
UPDRS_TO_XLS converts MER UPDRS 'Data' into a .xls named
...
%}
    ...
    status = create_updrs_template(Headers,1,File);
    if ~status
        comment = [File.name ': updrs_to_xls create_updrs_template failed'];
        close(w);
        return
    end
    ...
    if ~status
        comment = [File.name ': updrs_to_xls fill_patient_info write failed'];
        close(w);
        return
    end
    ...
    if ~status
        comment = [File.name ': updrs_to_xls fill_crw_info failed'];
        close(w);
        return
    end
    ...
    status = fill_updrs_info(DbsData,File);
    if status == 0
        comment = [File.name ': updrs_to_xls fill_updrs_info write failed'];
        close(w);
        return
    end
    ...
    for iTrack = 1:size(DbsData.data1,3)
        ...
        if ~status
            comment = [File.name ': updrs_to_xls create_track_template ' iTrack ' failed'];
            close(w);
            return
        end
        status = fill_recording_track(DbsData,cellIndex,iTrack,File);
        if ~status
            comment = [File.name ': updrs_to_xls fill_recording_track ' iTrack ' failed'];
            close(w);
            return
        end
    ...
    for iTrack = 1:size(DbsData.data31,3)
        ...
        if ~status
            comment = [File.name ': updrs_to_xls create_stim_template ' iTrack ' failed'];
            close(w);
            return
        end
        status = fill_stim_track(DbsData,cellIndex,iTrack,File);
        if ~status
            comment = [File.name ': updrs_to_xls fill_stim_track ' iTrack ' failed'];
            close(w);
            return
        end
